...
| Completion action and exit | ACTIVE | `lib/features/form_submission/presentation/widgets/bottom_sheet.widget.dart`, `form_submission_screen.widget.dart` | The bottom sheet returns one typed action. The form screen awaits it, then explicitly handles draft exit, finalization, route pop, and scope drop. A widget test proves selecting an action closes only the sheet and returns the action to its host. | High | Removes timing-dependent double navigation and makes scope teardown order visible before form-graph disposal is added. |
