...
| INCOMPLETE-FORWARD-CONTRACT | `lib/core/form/rule/validation_rule.dart`, `lib/core/form/element_template/field_template.entity.dart` | `validationRule` is the intended replacement for `rules[action=Error]`, but the app currently only parses/serializes it; it is not registered in dependency resolution or evaluation. The live fixture set contains 10 entries: nine have exact generated compatibility `Error` copies, while malaria `age` has a newer `validationRule` (`<= 0` or `> 100`) and a stale compatibility `Error` (`> 150`). | High | Implement `validationRule` as the sole validation-expression authority before removing compatibility output. Do not evaluate both representations independently. |
| ACTIVE-DEPRECATED-COMPATIBILITY | `lib/core/form/rule/action.dart`, `lib/features/form_submission/application/element/rule.extension.dart` | The current app evaluates `RuleActionType.Error`; this is the only active expression-validation path for deployed clients. Live forms prove the server is usually copying `validationRule` into this representation. | High | The server must continue emitting compatibility `Error` actions while deployed app versions do not evaluate `validationRule`. Remove the server copy only after the new app behavior is deployed and the minimum supported field version has advanced. |
