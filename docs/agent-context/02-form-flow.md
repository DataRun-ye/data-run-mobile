...
| --- | --- | --- | --- | --- | --- |
| Form and row navigation | ACTIVE | `lib/app/stacked/app.dart`; `lib/features/form_submission/presentation/section/repeat_table.widget.dart` | `FormSubmissionScreen` and `FormFlowBootstrapper` are generated routes. Repeat editing has one separate entry path: the active table pushes a context-owned `MaterialPageRoute<RepeatRowEditResult>` for `EditRowScreen`, waits for route completion, then applies the result; its unused generated route was removed. | High | Repeat editor lifetime is owned by the table's materialize/navigate/result/dematerialize sequence, not generated route arguments or GetX route timing. |
