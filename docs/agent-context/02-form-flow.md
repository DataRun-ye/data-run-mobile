...
| Completion action and exit | ACTIVE | `lib/features/form_submission/presentation/widgets/bottom_sheet.widget.dart`, `form_submission_screen.widget.dart`, `lib/features/form_submission/application/form_scope.dart` | The bottom sheet returns one typed action. Draft/final exit, clean back, and the read-only FAB all leave through `closeFormScope`; focused tests prove typed sheet return and GetIt disposer execution. | High for explicit exits | Removes timing-dependent navigation and the read-only exit leak, while establishing one teardown boundary for form-graph disposal. |
