...
| Per-submission DI scope | ACTIVE | `lib/features/form_submission/application/form_flow_bootstrapper_controller.dart` | Pushes a GetIt scope named by `dataInstance.id`, registers `FormTemplateRepository`, and registers `FormInstance` with its graph/control disposer. | High | Repeat state is held in a scoped in-memory `FormInstance`; its element streams, dependency links, controls, and field keys are released with the route-owned scope. |
