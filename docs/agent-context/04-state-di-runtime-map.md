...
| `stacked`, `stacked_services`, `stacked_generator` | ACTIVE | Declared in `pubspec.yaml`; `@StackedApp`, generated routing/services, and five active viewmodel pairs remain for startup, login, config sync, activity loading, and form bootstrap. Passive settings/drawer/dialog wrappers were removed and sync badges now use Riverpod. | High | Navigation remains active even as presentation state is consolidated incrementally. |
...
| Home/activity shell and drawer sync entry | ACTIVE | `home_wrapper_page.dart`, `app_drawer.dart`, `app_drawer_sync_item.dart`, `activity_list_view.dart` | Activity loading still uses a Stacked viewmodel; the drawer is now a stateless widget and its sync item routes to `SyncResourcesView`. | Medium/High | This is part of reaching synced/offline data-collection flows. |
