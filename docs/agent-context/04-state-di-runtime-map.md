...
| `stacked`, `stacked_services`, `stacked_generator` | ACTIVE | Declared in `pubspec.yaml`; `@StackedApp`, generated routing/services, and two active viewmodel pairs remain for config sync and form bootstrap. Login now uses an explicitly owned controller with `reactive_forms`; startup is a one-shot coordinator; passive UI and read projections use stateless/Riverpod ownership. | High | Navigation remains active even as presentation state is consolidated incrementally. |
