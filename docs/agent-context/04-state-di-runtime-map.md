...
| Submission table query service | ACTIVE | `lib/core/auth/auth_manager.dart`, `lib/features/data_instance/application/submission_table_service.dart` | Registered as a user-scoped factory with the scoped `AppDatabase`; submission table providers/widgets use it for filtered count and pagination. | High | Submission-list reads no longer appear to be form-engine state or resolve their database through an implicit field initializer. |
