...
| Dependency bootstrap | ACTIVE | `lib/app/di/injection.dart` | `configureDependencies()` calls `setupLocator()`, `setupGlobalDependencies(appLocator)`, then `registerDatabaseDependencies(appLocator)`. | High | This is the production DI order. |
...
| Stacked dialog service | ACTIVE | `lib/app/stacked/app.locator.dart`, `lib/commons/errors_management/d_exception_reporter.dart` | Generated setup registers `DialogService`; `DExceptionReporter` retrieves it and calls the generic `showDialog`. Unused `SnackbarService`, `BottomSheetService`, and custom dialog/sheet registrations were removed after finding no consumers. | High | Error reporting still depends on the generic dialog service; form completion uses Flutter's local `showModalBottomSheet`, not Stacked. |
...
| Error dialog/form completion sheet | ACTIVE | `lib/commons/errors_management/d_exception_reporter.dart`, `lib/features/form_submission/presentation/form_submission_screen.widget.dart` | Error reporting uses generic `DialogService.showDialog`; form completion calls Flutter's `showModalBottomSheet` directly. The uncalled custom `InfoAlertDialog` and `NoticeSheet` paths were removed. | High | These two active presentation paths now have explicit owners without generated alternative registrations. |
...
| OBSOLETE-REMOVED | Commented form-state adjunct providers/widgets | Element-properties, submission-creation, popup-section, old org-unit field, the repeat panel widget, and its later misleading enum-only file had no separate executable responsibility. Active row-edit results now have a narrowly named presentation type. | High | Active draft creation, form rendering, org-unit fields, and repeat editing use separate paths. |
