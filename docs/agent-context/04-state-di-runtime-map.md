...
| `stacked`, `stacked_services`, `stacked_generator` | ACTIVE | Declared in `pubspec.yaml`; `@StackedApp`, generated routing/services, and three active viewmodel pairs remain for login, config sync, and form bootstrap. Startup is now an explicit one-shot coordinator; passive settings/drawer/dialog wrappers, sync badges, and activity loading use stateless/Riverpod ownership. | High | Navigation remains active even as presentation state is consolidated incrementally. |
