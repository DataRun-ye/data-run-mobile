...
| `stacked`, `stacked_services`, `stacked_generator` | ACTIVE | Declared in `pubspec.yaml`; `@StackedApp`, generated routing/services, and four active viewmodel pairs remain for startup, login, config sync, and form bootstrap. Passive settings/drawer/dialog wrappers, sync badges, and activity loading now use stateless/Riverpod ownership. | High | Navigation remains active even as presentation state is consolidated incrementally. |
