...
These rows record the resolved ownership decisions from this pass.

| Area | Candidate duplicate paths | Current evidence |
| --- | --- | --- |
| Routing | Generated Stacked router only | The unused `app_routes` experiment was removed. |
| Login UI | `LoginView` only | The unrouted duplicate `LoginScreen` was removed. |
