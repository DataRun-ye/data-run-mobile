...
| Tests | `flutter test --no-pub` | PASS | All 23 current tests pass. They cover schema 3/4 to 5 migration, sync registration, repeat metadata/validation/metrics, submission completion time, assignment ownership, and managed-team scoping. |
| Static analysis | `flutter analyze --no-pub` | NO ERRORS; LINT DEBT | Reports 117 findings after the first ownership cleanup: 8 strict raw-type warnings and 109 style/info findings. Most were previously hidden under the nested SDK analyzer boundary and became visible after consolidation. Do not increase this count; clean it in bounded ownership slices rather than mixing it into mechanical moves. |
...
- The 23 tests are targeted characterization checks, not broad product coverage.
