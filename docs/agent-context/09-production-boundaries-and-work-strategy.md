...
Common Dio failures now cross one typed presentation boundary. Connectivity and timeout failures retain their category, login `401` is presented as invalid credentials, session authorization failures are presented as session expiry, cancellations remain silent, and internal server/exception details are not shown to users. Existing Arabic and English `S` localizations are authoritative. The shared async fallback, form bootstrap, assignment form opening, scanner, and compact value display no longer render raw exception or plugin details. `ServerFailure` decodes the server's current nested errors, RFC-style and plain-text compatibility responses, and the planned `code`/`args`/trace shape. Proven security and submission codes `E3000`-`E3006` and `E4110`-`E4116` have mobile-owned messages; unknown actionable `400` details remain readable. `test/dev/error_presentation_test.dart` characterizes these boundaries.

Submission bulk upload now returns one typed outcome: nothing eligible, complete, partial, rejected, or request failure. Every prepared row leaves `uploading`; accepted rows become `synced`, while rejected, omitted, malformed-response, and transport-failed rows become `syncFailed`. New failures are stored as versioned category/server-code snapshots in the existing `lastSyncMessage` column and localized only when displayed; legacy raw rows fall back to a generic localized sync error. No schema, request payload, retry, or server contract changed. `test/dev/submission_upload_service_test.dart` covers these transitions.

Build generation is constrained by ownership. Production uses only the Stacked router and locator generated from `lib/app/stacked/app.dart`; its form, logger, dialog, and bottom-sheet generators are disabled. Previously all six auto-applied to generated Dart outputs and formed chained variants consumed by Drift and other builders. After constraining them, a clean full generation fell from about 497 seconds to 95 seconds including one-time builder compilation, and a no-change run takes about 5 seconds. Active router, locator, DI, Drift, and Freezed outputs remained byte-identical; replacing those libraries is not justified by current evidence.

| Finding | Classification | Evidence | Required next proof |
|---|---|---|---|
| Server domain error responses remain structurally inconsistent | `ACTIVE-SUPPORT` residual UX risk | Mobile recognizes the server `ErrorCode` values used by active security/submission paths, but current responses still mix RFC7807 fields, nested error objects, and plain text; server arguments and trace IDs are not returned consistently | Add `code`, `args`, and trace ID additively on the server, and add structured bulk `failureDetails` without removing the legacy string map |
