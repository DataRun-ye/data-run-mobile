...
|---|---|---|---|
| Startup refresh failure does not distinguish a rejected refresh token from offline/transient failure | `ACTIVE-CORE` recovery-policy risk | `initializeApp` can throw from connectivity or `TokenRefresher`; `AuthManager.initialize` currently maps every failure to unauthenticated while retaining cached credentials | Characterize offline startup and server-rejected refresh separately; preserve recoverable offline access evidence while permanently rejected credentials use the established revocation transition |
