...
|---|---|---|---|
| Login, session restoration, token refresh/expiry, and logout have not received a focused ownership and failure-mode review | `ACTIVE-CORE` correctness risk | Refresh/session behavior is split across `auth_manager.dart`, `token_refresher.dart`, `auth_interceptor.dart`, and `token_storage.dart` | Trace startup/login/restore/401/concurrent-request/offline/refresh-failure/logout paths; characterize token rotation, retry limits, local-data retention, and user-visible recovery before changing ownership |
