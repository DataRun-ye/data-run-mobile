...
Token refresh now has one network and rotation owner. `AuthApi` calls the server's active `/api/v1/refresh` endpoint, `TokenRefresher` serializes concurrent refreshes and persists the rotated pair, and `AuthInterceptor` retries with that returned pair. Logout captures the active user before clearing its marker and tokens, while retaining the cached profile and per-user database. `test/dev/auth_token_lifecycle_test.dart` covers rotation, concurrent requests, authenticated retry, credential revocation, and local-profile retention.

| Finding | Classification | Evidence | Required next proof |
|---|---|---|---|
| Request-time refresh failure revokes stored credentials but does not yet invalidate `AuthManager`, close the active user scope, clear user-scoped telemetry, or route to login | `ACTIVE-CORE` correctness risk | `auth_interceptor.dart` clears `AuthStorage`; session and navigation ownership remain in `auth_manager.dart` | Define one session-revocation transition and characterize its behavior during foreground requests, concurrent failures, offline startup, and logout |
| Login activates the per-user scope before session and credential persistence completes | `ACTIVE-CORE` rollback risk | `_activateUserSession` precedes `storeSession` and `saveCredentials` in `auth_manager.dart` | Add a failing-storage characterization and make activation transactional without deleting the retained database |
| Common server, network, login, and synchronization failures are not modeled and presented as readable localized messages | `ACTIVE-SUPPORT` UX and support risk | Active failure paths expose low-level exceptions through generic popup/error handling; no single translation boundary is established | Inventory active server error DTOs and common Dio/connectivity/auth failures, then define one typed propagation-to-presentation contract with Arabic and English characterization |
| Configuration synchronization is disruptive on slow or unreliable connections | `ACTIVE-CORE` offline UX and reliability risk | Login/session activation invokes the sequential configuration datasource pipeline; progress, retry, partial-success, resume, and cancellation behavior have not been characterized | Trace the active sync coordinator under latency, timeout, reconnect, and partial failure before changing retry or scheduling policy |
| `data_elements` is synchronized and schema-backed, but an active read consumer has not yet been proven | `UNKNOWN-SCHEMA-RISK` | Its datasource/table remain in active configuration registration while form rendering primarily consumes whole template JSON | Prove or reject runtime reads; if source-dead, remove registration/source first and drop the table only through schema-3/4/5 migration fixtures |
