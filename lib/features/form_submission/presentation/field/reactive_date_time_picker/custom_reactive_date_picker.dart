    ...
    switch (type) {
      case ReactiveDatePickerFieldType.date:
        return _DefaultAccessor(
            dateFormat ?? DateFormat(DateHelper.UI_DATE_FORMAT));
      case ReactiveDatePickerFieldType.time:
        return _DefaultAccessor(
            dateFormat ?? DateFormat(DateHelper.TIME_FORMAT));
      case ReactiveDatePickerFieldType.dateTime:
        return _DefaultAccessor(
            dateFormat ?? DateFormat(DateHelper.DATE_TIME_FORMAT_EXPRESSION));
      case ReactiveDatePickerFieldType.month:
        return _DefaultAccessor(dateFormat ?? DateFormat('yMMM'));
      case ReactiveDatePickerFieldType.week:
        return _DefaultAccessor(dateFormat ?? DateFormat.yMEd('en_US'),
            isWeek: true);
      case ReactiveDatePickerFieldType.year:
        return _DefaultAccessor(dateFormat ?? DateFormat(DateFormat.YEAR));
    }
  }
...
class _DefaultAccessor extends ControlValueAccessor<String, String> {
  _DefaultAccessor(this.fmt, {this.isWeek = false});
  ...
  String? modelToViewValue(String? modelValue) {
    ...
    if (isWeek) return '${dt.year}, W${getIsoWeekNumber(dt)}';
      ...
      return Dialog(
        child: CalendarDatePicker2(
          ...
          onValueChanged: (dates) {
            final picked = dates.firstOrNull;
            if (picked != null)
